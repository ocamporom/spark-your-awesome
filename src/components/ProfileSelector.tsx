...
import jag from "@/pictures/jag.jpg";
    ...
    <div className="min-h-[600px] flex flex-col items-center justify-center gap-12 py-12">
      ...
      >
        {profiles.map((profile, index) => {
          ...
          const collapsedSpacing = 70;
          const expandedSpacing = 280;
          const centerIndex = (profiles.length - 1) / 2;

          const translateX = isHovered
            ? (index - centerIndex) * expandedSpacing
            : (index - centerIndex) * collapsedSpacing;

          return (
            <Card
              key={profile.id}
              className={`absolute cursor-pointer transition-transform duration-500 ease-[cubic-bezier(.22,1,.36,1)] hover:scale-105 ${
                isHovered ? "hover:-translate-y-5 hover:z-50" : ""
              }`}
              style={{
                width: "260px",
                height: "340px",
                ...
                >
                  <div className="w-32 h-32 rounded-full overflow-hidden bg-white/20 backdrop-blur-sm flex items-center justify-center">
                    ...
                    {profile.name === "John Michael Lagumbay" ? (
                      <img src={jm} alt="John Michael Lagumbay" className="w-full h-full object-cover" />
                    ) : profile.name === "John Paul Lagumbay" ? (
                      <img src={jp} alt="John Paul Lagumbay" className="w-full h-full object-cover" />
                    ) : profile.name === "Jay Are Galinada" ? (
                      <img src={jag} alt="Jay Are Galinada" className="w-full h-full object-cover" />
                    ) : profile.name === "Rom Joses Ocampo" ? (
                      <img src={rom} alt="Rom Joses Ocampo" className="w-full h-full object-cover" />
                    ) : (
